...
the new bcc reference `w50mo25cr25_4x4x4.yaml` ($\mathrm{W_{64}Mo_{32}Cr_{32}}$, 128 sites) runs end to end, and
...
The discovery: for this composition **$E_{\mathrm{floor}} = 0$ exactly**. The fractions $(\tfrac{1}{2}, \tfrac{1}{4}, \tfrac{1}{4})$ are dyadic
and every $\tilde{x}\cdot P_t(n)$ on bcc 4×4×4 is integral, so every quantization term vanishes — a perfect
SQS is not excluded by counting. The floor code handles it correctly (the K = 2 $full = 2\times diag$
...
$E_{\mathrm{pure}} \le 1.2 \times 10^{-1}$ gate.
...
robustly: **rc = 0 SUCCESS at $2.50 \times E_{\mathrm{floor}}$** (8.111947e-3, Cm) in 19 s wall on the dev
...
reaches **$E_{\mathrm{pure}} = 1.330 \times 10^{-1}$ (Cm)** at generation 45 with 68 accepted migrations over 18,610
