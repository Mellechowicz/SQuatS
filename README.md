...
| `error.normalize` | `true` | `false` | divide the pair sector by the analytic `E[random]` (SPEC 4.3): `E = 1` is the random alloy, and `E_floor` / `e_tol: auto` follow the same units |
...
| `error.normalize: true` + multiplet sectors | only the pair sector is divided by `E[random]`; the lambda-weighted sectors are added in raw units |
| `error.normalize` compared across runs | absolute errors are in different units than an unnormalized run — never mix the two in one table |
...
`state: trajectory signature mismatch` — `error.normalize` included. The
`multiplets.shell3`/`shell4` keys are signed only while a lambda is non-zero,
so they are free to change on a pair-only run (where they have no effect).
